...
use App\Models\Wishlist; // NAMESPACE: Menggunakan App\Models\Wishlist
use App\Models\WishlistItem; // NAMESPACE: Menggunakan App\Models\WishlistItem
...
$addToWishlist = function($productBranchId) {
    // Pastikan user sudah login
    if (!Auth::check()) {
        $this->js("alert('Silakan masuk untuk menambahkan produk ke Wishlist.')");
        return;
    }
    ...
    if (!$productBranch) {
        $this->js("alert('Produk tidak ditemukan.')");
        return;
    }

    // 1. Cari atau buat Wishlist untuk user yang sedang login
    $wishlist = Wishlist::firstOrCreate(
        ['user_id' => Auth::id()], // user_id sekarang mass assignable
        ['name' => 'Default Wishlist'] 
    );

    // 2. Cek apakah produk sudah ada di wishlist
    $exists = WishlistItem::where('wishlist_id', $wishlist->id)
                          ->where('product_branch_id', $productBranchId)
                          ->exists();

    if ($exists) {
        $this->js("alert('Produk ini sudah ada di Wishlist Anda.')");
    } else {
        ...
        $this->js("alert('Produk berhasil ditambahkan ke Wishlist!')");
        // Dispatch event untuk memberitahu komponen lain untuk refresh
...
<div class="row">
    <style>
        ...
        .action-buttons {
            display: grid;
            grid-template-columns: 1fr 1fr 1fr; /* Dipertahankan untuk 3 item: Sewa, Wishlist, Keranjang */
            gap: 0.5rem;
        }
        ...
        /* CSS BARU UNTUK WISHLIST */
        .btn-wishlist { 
            background: rgba(220, 53, 69, 0.1); /* Merah muda */
    ...
    <div class="col-lg-3 filter-column">
        <div class="filter-card card bg-light mb-4">
            ...
            <div class="card-body">
                ...
                <div class="filter-section">
                    <x-form-group name="brand" label="Merek">
                        <x-form-select
                            name="brand"
                            class="form-select form-select-solid fw-bold"
                ...
                <div class="product-card card h-100">
                    ...
                    <div class="card-body" wire:ignore.self>
                        ...
                        <div class="action-buttons">
                            ...
                            {{-- Tombol Wishlist (Icon Love) --}}
                            <button class="btn-action btn-wishlist" 
                                    wire:click="addToWishlist({{ $variations->id }})" 
